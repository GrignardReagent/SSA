"""
IY036 (1/2) v2, Eddie HPC submission variant. Launched via
`qsub IY036_supcon_allclass_training_eddie_v2.sh` on the University of
Edinburgh Eddie cluster. The v1 scripts are left untouched as the record of
...
wandb_config = {
    ...
    "name": f"supcon_allclass_eddie_v2_b{batch_size}_lr{lr}_d{dropout}_L{num_layers}_H{nhead}_D{d_model}_temp{temperature}_{timestamp}",
...
out_path = IY036_DIR / f"IY036_v2_supcon_allclass_final_results_eddie_{timestamp}.csv"
...
comparison.to_csv(
    IY036_DIR / f"IY036_v2_supcon_allclass_comparison_eddie_{timestamp}.csv", index=False)

# write training history to csv
pd.DataFrame({k: pd.Series(v) for k, v in history.items()}).to_csv(
    IY036_DIR / f"IY036_v2_supcon_allclass_history_eddie_{timestamp}.csv", index=False)
