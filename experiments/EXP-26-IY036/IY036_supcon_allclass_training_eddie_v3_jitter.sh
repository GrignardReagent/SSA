...
# Job name
#$ -N IY036_supcon_allclass_training_eddie_v3_jitter
#$ -o IY036_supcon_allclass_training_eddie_v3_jitter.o$JOB_ID
#$ -e IY036_supcon_allclass_training_eddie_v3_jitter.e$JOB_ID
...
# job scripts; both call the same .py. Budget for this arm: ~28 h (3 seeds x ~9.3 h).
export AUG_RECIPE=jitter_only

export RUN_TIMESTAMP=$(date +%Y%m%d_%H%M%S)

# Run Python script and log output
python IY036_supcon_allclass_training_eddie_v3.py > "IY036_supcon_allclass_training_eddie_v3_jitter_${RUN_TIMESTAMP}.out" 2>&1
