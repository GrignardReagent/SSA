...
# Job name
#$ -N IY036_supcon_allclass_training_eddie_v3_composed
#$ -o IY036_supcon_allclass_training_eddie_v3_composed.o$JOB_ID
#$ -e IY036_supcon_allclass_training_eddie_v3_composed.e$JOB_ID
...
# The A/B variable. This is the ONLY line that differs between the two v3
# job scripts; both call the same .py. Budget for this arm: ~35 h (3 seeds x ~11.8 h).
export AUG_RECIPE=composed

export RUN_TIMESTAMP=$(date +%Y%m%d_%H%M%S)

# Run Python script and log output
python IY036_supcon_allclass_training_eddie_v3.py > "IY036_supcon_allclass_training_eddie_v3_composed_${RUN_TIMESTAMP}.out" 2>&1
