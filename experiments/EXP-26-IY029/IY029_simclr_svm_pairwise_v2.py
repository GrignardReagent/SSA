"""
IY029 v2: SimCLR Pairwise SVM — All SimCLR Models (IY029 datasets, n=14 000)

Code is unchanged from IY029_simclr_svm_pairwise.py; only data paths point
to the regenerated IY029 static .pt files and output filenames carry a _v2 suffix.

*** Designed to run on the University of Edinburgh Eddie HPC cluster. ***
Submit with: qsub IY029_simclr_svm_pairwise_v2.sh  (from the EXP-26-IY029 dir)

Evaluates all SimCLR checkpoints on a pairwise same/different task using
IY029 (2-fold) and IY029 (10-fold) variation datasets across four conditions
...
Saves:
  IY029_simclr_svm_pairwise_v2_results.json
  IY029_simclr_svm_pairwise_v2_results_combined.json
  IY029_simclr_svm_pairwise_v2_2fold.png
  IY029_simclr_svm_pairwise_v2_10fold.png
  IY029_simclr_svm_pairwise_v2_best_model.png
  IY029_simclr_svm_pairwise_v2_combined_2fold.png
  IY029_simclr_svm_pairwise_v2_combined_10fold.png
  IY029_simclr_svm_pairwise_v2_combined_best_model.png
"""
import os
...
MODEL_REGISTRY = [
    # Registry entries are (checkpoint_path, short_plot_label); sorting makes
    # repeated runs deterministic even when the filesystem returns glob results
...
def load_pair_data(train_pt: Path, test_pt: Path):
    """
    Load IY029 static pairwise loader.
    ...
    def _extract(pt_path):
        ...
        Xs, ys = [], []
        for X, y in loader:
            # The static loaders already contain paired trajectories, so no
            # resampling or pair construction happens in this evaluation script.
            Xs.append(X.numpy())
...
def encode_pairs(model: SSL_Transformer, X_np: np.ndarray,
    ...
    with torch.no_grad():
        for i in range(0, len(X_np), batch_size):
            # Encode each half independently to match the SimCLR pretraining
            # view semantics, then concatenate the two learned representations.
...
def encode_pairs_combined(model: SSL_Transformer, X_np: np.ndarray,
    ...
    with torch.no_grad():
        for i in range(0, len(X_np), batch_size):
            # Reuse the same SimCLR embeddings as the embedding-only condition;
            # raw flattened trajectories are appended after encoding.
            b1 = torch.tensor(x1[i:i+batch_size], dtype=torch.float32).to(device)
            b2 = torch.tensor(x2[i:i+batch_size], dtype=torch.float32).to(device)
            z1_parts.append(model.backbone.encode(b1).cpu().numpy())
            z2_parts.append(model.backbone.encode(b2).cpu().numpy())
...
def run_svm(model: SSL_Transformer,
    ...
    # Fit scaling on training features only to avoid test-set leakage.
...
def plot_fold(results: dict, model_labels: list, ds_names: list,
    ...
    sorted_labels = sorted(
        model_labels,
        # Sort by mean dataset accuracy within this fold so the strongest bars
        # appear first without hiding condition-specific differences.
...
def plot_best_model(results: dict, model_labels: list, ds_names: list,
    ...
    # The best model is selected across both fold magnitudes and all datasets.
    ...
    for ax, accs, fold_title in [
        (axes[0], accs_2fold,  'IY029 2-fold variation'),
        (axes[1], accs_10fold, 'IY029 10-fold variation'),
    ]:
...
def run_evaluation_loop(prepared_data: dict, ds_names: list,
    ...
    """
    Run all models × 4 datasets × 2 folds and return nested results dict.
    ...
    for i, (ckpt_path, mdl_label) in enumerate(MODEL_REGISTRY):
        ...
        for ds_name in ds_names:
            ...
            for fold in ('iy011', 'iy014'):
                # prepared_data stores (X_train, X_test, y_train, y_test) for
                # each dataset/fold combination to avoid repeated disk reads.
...
def main():
    ...
    for cfg in DATASET_CONFIGS:
        ...
        # Load once, then reuse for both embedding-only and combined-feature SVMs.
        prepared_data[name] = {
    ...
    save_path = SCRIPT_DIR / 'IY029_simclr_svm_pairwise_v2_results.json'
    ...
    plot_fold(results, model_labels, ds_names, ds_colors,
              'iy011', 'IY029 2-fold variation',
              SCRIPT_DIR / 'IY029_simclr_svm_pairwise_v2_2fold.png')
    plot_fold(results, model_labels, ds_names, ds_colors,
              'iy014', 'IY029 10-fold variation',
              SCRIPT_DIR / 'IY029_simclr_svm_pairwise_v2_10fold.png')
    plot_best_model(results, model_labels, ds_names, ds_colors,
                    SCRIPT_DIR / 'IY029_simclr_svm_pairwise_v2_best_model.png',
    ...
    save_path_combined = SCRIPT_DIR / 'IY029_simclr_svm_pairwise_v2_results_combined.json'
    ...
    plot_fold(results_combined, model_labels, ds_names, ds_colors,
              'iy011', 'IY029 2-fold variation',
              SCRIPT_DIR / 'IY029_simclr_svm_pairwise_v2_combined_2fold.png',
              title_prefix='Pairwise SVM [raw + encoded]')
    plot_fold(results_combined, model_labels, ds_names, ds_colors,
              'iy014', 'IY029 10-fold variation',
              SCRIPT_DIR / 'IY029_simclr_svm_pairwise_v2_combined_10fold.png',
              title_prefix='Pairwise SVM [raw + encoded]')
    plot_best_model(results_combined, model_labels, ds_names, ds_colors,
                    SCRIPT_DIR / 'IY029_simclr_svm_pairwise_v2_combined_best_model.png',
