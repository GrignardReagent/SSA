...
"""
Simulate a CV control dataset where t_ac and MEAN are FIXED, 
but CV varies. This effectively isolates 
CV as the only distinguishing feature.
...
# Results log CSV
results_path = os.path.join(data_dir, "IY014_simulation_cv_parameters_sobol.csv")
...
print(f"=== Generating CV Control Dataset ===")
print(f"Fixed Parameters: t_ac={TAC_FIXED}, MU={MU_FIXED}")
print(f"Varying CV: [{CV_MIN}, {CV_MAX}] via Sobol sampling")
...
for combination_idx, (mu, t_ac, cv) in enumerate(
    tqdm(zip(mu_target, t_ac_target, cv_target), 
         total=total_combinations, desc="Simulating Sobol samples (cv variation)"),
        start=1,
):
    record = {
        "mu_target": mu,
...
plt.legend()
plt.grid(True)
plt.savefig(f'{data_dir}/IY014_mu_target_vs_mu_observed.png', dpi=300)
plt.show()
...
plt.legend()
plt.grid(True)
plt.savefig(f'{data_dir}/IY014_cv_target_vs_cv_observed.png', dpi=300)
plt.show()
...
plt.legend()
plt.grid(True)
plt.savefig(f'{data_dir}/IY014_t_ac_target_vs_t_ac_observed.png', dpi=300)
plt.show()