#!/usr/bin/env python3
"""Benchmark TFTransformer on clean vs noise-corrupted synthetic data.

The script synthesises a modest set of telegraph-model trajectories, trains the
`TFTransformer` on the clean sequences and then evaluates the trained
...
def main() -> None:
    ...
    model = TFTransformer(ModelCfg(n_classes=2))
