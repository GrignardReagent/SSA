#!/usr/bin/env python3
"""
Simulate a mini batch of Sobol sequence samples with mean (mu) and Autocorrelation time (t_ac) fixed.

This script is adapted from ``IY010_simulation_7_mu_cv_fixed.py``. The only difference
is that the target mean ``mu`` and ``t_ac`` are fixed to a constant value, while CV values are sampled using a Sobol sequence.
This should be an easier problem to solve compared to ``IY010_simulation_7_mu_cv_fixed.py``, as the classifier don't need to learn the temporal patterns like the autocorrelation time. 
See IY007_find_tilda.py for a similar experiment

- ``t_ac`` is fixed to ``T_AC_FIXED``
...
# Configuration
# -----------------------------------------------------------------------------
CV_FIXED = 2.0  # constant CV value for all runs
T_AC_FIXED = 5.0   # constant autocorrelation time value for all runs

# Create data directory if it doesn't exist
# Each simulation script writes to its own folder to avoid overwriting results
# from other experiments.
data_dir = "data_7_cv_t_ac_fixed"
...
# ``t_ac`` in [0.5, 100] 
t_ac_target = np.full(N, T_AC_FIXED)  # same ``t_ac`` for all samples
cv_target = np.full(N, CV_FIXED)  # same ``cv`` for all samples
...
for combination_idx, (mu, t_ac, cv) in enumerate(
    ...
    results_path = os.path.join(data_dir, "IY010_simulation_parameters_7_cv_t_ac_fixed.csv")
