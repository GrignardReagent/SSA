#!/usr/bin/env python3
"""
Sample parameter space using np.linspace.

This script uses the solve_tilda_parameters function to simulate trajectories used for training transformer models, it does the following:
...
data_dir = "data_3" # different folder to the first attempt.
...
success_count = 0
failure_count = 0
total_combinations = len(mu_target) * len(t_ac_target) * len(cv_target)  # Full grid combinations
all_records = []
...
for combination_idx, (mu, t_ac, cv) in enumerate(
    tqdm(itertools.product(mu_target, t_ac_target, cv_target), total=total_combinations, desc='Simulating parameter grid'),
    start=1):
    ...
    results_path = os.path.join(data_dir, "IY010_simulation_parameters_3.csv")
