#!/usr/bin/env python3
"""
Sample parameter space using Sobol sequences to test find_tilda_parameters.
...
- Use a Sobol sequence, 1000 statistical properties
- CV target: 0.5 to 2.0.
- Timepoint: max(144, int(t_ac * 20))
- Size: 50 (Each additional trajectory from the same parameter point provides diminishing returns: beyond ~20–50 trajectories, you mostly re-sample noise rather than expose the model to new dynamics. If simulation time is a constraint, it’s generally more effective to invest in more Sobol points—i.e., broader coverage of parameter space—than in many repeated trajectories per point. )
"""
...
data_dir = "data_6" # different folder to the first attempt.
os.makedirs(data_dir, exist_ok=True)

# Define parameter ranges and other simulation parameters
# build Sobol seq (better space‑filling with fewer points)
N = 1000 
# Build a Sobol engine (base-2 digital net)
sobol = qmc.Sobol(d=3, scramble=True, seed=42)
U = sobol.random_base2(int(np.ceil(np.log2(N))))[:N]  # N points in [0,1)^3, cut off till N.
# Map each dimension to the parameter ranges
mu_target  = qmc.scale(U[:,0:1], [1], [10_000])      # map to [1, 10000]
t_ac_target = qmc.scale(U[:,1:2], [0.5], [100])      # map to [0.5, 100]
cv_target  = qmc.scale(U[:,2:3], [0.5], [2.0])      # map to [0.5, 2.0]
...
for combination_idx, (mu, t_ac, cv) in enumerate(
    ...
    try:
        ...
        time_points = np.arange(0, 
                                max(144, int(t_ac * 20)), # need to set a minimum timespan so that for small t_ac, there are still enough timepoints
                                1.0)
        size = 50 # TODO: may need to reconsider the choice of this to be adaptive.
    ...
    results_path = os.path.join(data_dir, "IY010_simulation_parameters_6.csv")
