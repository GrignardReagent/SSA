...
# Results log CSV
results_path = os.path.join(data_dir, "IY014_simulation_t_ac_parameters_sobol.csv")
...
if not RESULTS_PATH.exists():
    ...
    # Keep only successful rows with a real filename
    df_ok = df_params[df_params["success"] == True].copy()
    df_ok["trajectory_filename"] = df_ok["trajectory_filename"].astype("string")
    df_ok = df_ok.dropna(subset=["trajectory_filename"])
    df_ok = df_ok[df_ok["trajectory_filename"].str.len() > 0]
    ...
    traj_paths = [DATA_ROOT / fname for fname in df_ok['trajectory_filename']]
