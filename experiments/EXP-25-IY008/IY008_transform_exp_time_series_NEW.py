...
OUT_DIR = Path(__file__).parent / "4_transformed_exp_time_series_NEW"
...
TSV_DIR = Path(f"/home/ianyang/stochastic_simulations/experiments/EXP-25-IY008/2_wela_data_analysis/")
# WINDOW_SIZE = 30
# TOLERANCE = 0.10
...
if __name__ == "__main__":
    ...
    for omid in OMIDS:
        matches = [
            p for p in (TSV_DIR / str(omid)).glob(f"{omid}*.tsv")
            if "_post_media_switch" not in p.name
        ]
        if matches:
            tasks.append((str(omid), matches[0]))
        else:
            missing.append(omid)
