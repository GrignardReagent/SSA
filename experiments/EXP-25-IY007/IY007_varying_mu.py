...
"""Simulate telegraph model systems across mean ratios and benchmark classifiers."""
...
mu_target_normal = 100  # initial mean for normal condition
t_ac_target = 5  # Autocorrelation time (same for both conditions)

# Try to find parameters that work for all mean ratios
print("Finding parameters where all mean ratios are biologically appropriate...")
# Define mean ratio range
mu_ratio = np.arange(0.1, 2.0, 0.01)  # Range that's likely to be biologically appropriate

# Initial parameters are no longer required as find_tilda_parameters determines all kinetic rates
print("\nStarting simulations with:")
print(
    f"CV: {target_cv:.2f}, Normal Mean: {mu_target_normal:.2f}, Autocorrelation Time: {t_ac_target:.2f}"
)

###############################################################################
# 2) Loop over different mean ratios
###############################################################################

# Track inappropriate ratios for reporting
inappropriate_ratios = []

for ratio in tqdm.tqdm(mu_ratio, desc="Running Mean Ratio Simulations"):
    # For the stress condition, we define mean by ratio
    mu_target_stress = ratio * mu_target_normal
    print(
        f"\nTesting mean ratio: {ratio:.2f}, Stress mean: {mu_target_stress:.2f}, Normal mean: {mu_target_normal:.2f}"
    )
    ...
    for condition in ["normal", "stress"]:
        # Decide which mean to use for this condition
        if condition == "normal":
            mu_for_condition = mu_target_normal
        else:  # condition == "stress"
            mu_for_condition = mu_target_stress

        try:
            # Solve for parameters using the tilda formulation
            rho, d, sigma_b, sigma_u = find_tilda_parameters(
                mu_target=mu_for_condition,
    ...
    for i in range(num_iterations):
        ...
        # Create output directory with mean values
        output_dir = f"/home/ianyang/stochastic_simulations/experiments/EXP-25-IY007/data_mu/mRNA_trajectories_mu_{mu_target_stress:.2f}_{mu_target_normal:.2f}"
        os.makedirs(output_dir, exist_ok=True)

        # Save full time series
        output_file = (
            f"{output_dir}/m_traj_mu_{mu_target_stress:.2f}_{mu_target_normal:.2f}_{i}.csv"
        )
        ...
        df_acc_results = pd.DataFrame(
            {
                ...
                "Stats": [stats],
                "Mean Ratio": [ratio],
                "Normal Mean": [mu_target_normal],
                "Stress Mean": [mu_target_stress],
        ...
        # Save results
        results_file = "data_mu/IY007_varying_mu.csv"
...
if len(inappropriate_ratios) == 0:
    print("\n✅ All mean ratios were biologically appropriate! No iterations were skipped.")
else:
    print(
        f"\n⚠️ {len(inappropriate_ratios)} out of {len(mu_ratio)} mean ratios were not biologically appropriate"
    )
    print(
    ...
    inappropriate_file = "data_mu/inappropriate_ratios.csv"
