...
from simulation.simulate_telegraph_model import simulate_two_telegraph_model_systems, extract_integer_timepoints_from_df
...
for ratio in tqdm.tqdm(cv_ratio, desc="Running CV Ratio Simulations"):
    ...
    for i in range(num_iterations):
        ...
        # we only want the integer time points
        integer_ss_df = extract_integer_timepoints_from_df(steady_state_series)
        # save the integer steady state data
        integer_ss_file = os.path.join(save_path, f"{os.path.splitext(os.path.basename(output_file))[0]}_SS_integer.csv")
        integer_ss_df.to_csv(integer_ss_file, index=False)

        # Check for duplicate column names and handle them
        if len(integer_ss_df.columns) != len(set(integer_ss_df.columns)):
            ...
            col_map = {}
            for col in integer_ss_df.columns:
                if col in col_map:
            ...
            # Rename columns
            integer_ss_df.columns = new_columns

        # Make sure 'label' is correctly formatted
        if 'label' in integer_ss_df.columns:
            stress_trajectories = integer_ss_df[integer_ss_df['label'] == 0].iloc[:, 1:].values
            normal_trajectories = integer_ss_df[integer_ss_df['label'] == 1].iloc[:, 1:].values
        else:
            ...
            label_col = integer_ss_df.columns[0]
            print(f"'label' column not found, using '{label_col}' as label column")
            stress_trajectories = integer_ss_df[integer_ss_df[label_col] == 0].iloc[:, 1:].values
            normal_trajectories = integer_ss_df[integer_ss_df[label_col] == 1].iloc[:, 1:].values

        stats = statistical_report(parameter_sets, stress_trajectories, normal_trajectories)

        # classifiers
        X_train, X_val, X_test, y_train, y_val, y_test = load_and_split_data(integer_ss_file, split_val_size=0.2)
