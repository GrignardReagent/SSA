...
No custom CUDA kernel (unlike the third-party `torchlars` package, which
requires compiling a CUDA extension at install time -- fragile on a shared or
HPC environment, and unmaintained since ~2020). This is plain tensor ops, so
it runs anywhere torch does.
