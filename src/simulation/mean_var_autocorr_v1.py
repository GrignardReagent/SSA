...
def equations(vars, sigma_u, mu_target=None, variance_target=None, autocorr_target=None, cv_target=None, fano_factor_target=None, symbolic=False):
    ...
    if cv_target is not None:
        ...
        #     ((sigma_u * sigma_b) * rho**2 / (d * (sigma_b + sigma_u + d) * (sigma_u + sigma_b)**2))
        # )) / mu_target) - cv_target
        # eqs.append(cv_eqn if symbolic else float(cv_eqn))
...
def find_parameters(parameter_set, mu_target=None, variance_target=None, autocorr_target=None, cv_target=None, fano_factor_target=None,
    ...
    for attempt in range(max_attempts):
        ...
        if attempt == 0: 
            ...
            factor = 1 + attempt * 0.3 # no upper limit on factor
            ...
            current_num_guesses = int(num_guesses * (1 + attempt * 0.2)) # no upper limit on num_guesses
