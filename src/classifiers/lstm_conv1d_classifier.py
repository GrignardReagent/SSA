...
def lstm_conv1d_classifier(X_train, X_val, X_test, y_train, y_val, y_test, 
                    ...
                    batch_size=32, epochs=50, patience=10, bidirectional=True, use_attention=False):
    """ 
    ...
    model = LSTMClassifier(input_size=input_size, hidden_size=hidden_size,
                           ...
                           dropout_rate=dropout_rate,learning_rate=learning_rate,bidirectional=bidirectional, use_conv1d=True, use_attention=use_attention, use_auxiliary=False)

    model.train_model(train_loader, val_loader, epochs=epochs, patience=patience)

    # === Evaluate ===
    lstm_acc = model.evaluate(test_loader)
    print(f"=== Long-Short Term Memory (LSTM) - Conv1D Accuracy: {lstm_acc:.2f} ===")
